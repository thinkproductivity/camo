...
(function() {
  var Crypto, Fs, Http, Https, Path, QueryString, Url, accepted_image_mime_types, camo_hostname, content_length_limit, cors, current_connections, debug_log, default_security_headers, error_log, finish, four_oh_four, hexdec, keep_alive, logging_enabled, max_redirects, port, process_url, server, shared_key, socket_timeout, started_at, total_connections, useCorsMiddleware, version,
  ...
  useCorsMiddleware = function(next) {
    return function(req, resp) {
      return cors({
        origin: '*'
      })(req, resp, function(err) {
        if (err) {
          throw err;
        }
  ...
  server = Http.createServer(useCorsMiddleware(function(req, resp) {
